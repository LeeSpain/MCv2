...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      ...
      // CEO LANDING LOGIC: Always force Dashboard path if trying to access the redundant ceo-dashboard path
      if (location.pathname === '/ceo-dashboard' && currentUser.role === Role.CEO) {
         shouldRedirect = true;
         target = '/dashboard';
      }
      // Security: Non-CEO trying to access CEO logic (though we unified it, we keep /dashboard as root)
    }
